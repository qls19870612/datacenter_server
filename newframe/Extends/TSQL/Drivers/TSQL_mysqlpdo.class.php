...
class TSQL_mysqlpdo
{
    ...
    {
        try {
            $this->conn = new PDO ('mysql:host=' . $this->Host . ";port=" . $this->Port . ';dbname=' . $this->DBName, $this->User, $this->PassWd);
            $this->conn->exec("SET NAMES  utf8");
            $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->state = true;
        } catch (PDOException $e) {
            $this->errorLog("pdo不可用：" . $this->DBName);
        }
    }
    ...
    public function parseArray($status, $table, $data, $where = null)
    {
        ...
        if (is_array($data)) {
            if ($status == 'insert') {
                ...
                return "INSERT INTO {$table} ($columnName)  VALUES ($value)";
