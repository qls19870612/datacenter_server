...
!function($){
    ...
    var renderDefault = {
        checkCol_fix : function(val,item,rowIndex){
            return '<input type="checkbox" class="mmg-check">';
        },
        indexCol_fix : function(val,item,rowIndex){
            return '<label class="mmg-index">' + (rowIndex+1) + '</label>';
        },
        fixed2 : function(val,item,rowIndex) {
            var Nval = Number(val) || 0;
            return Nval.toFixed(2);
        },
        highLight2 : function(val,item,rowIndex) {
            var Nval = Number(val) || 0;
            var cla = Nval > 0 ? 'hl-up' : 'hl-down';
            return '<span class="' + cla + '">' + Nval.toFixed(2) + '</span>'
        }
    }
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.checkCol){
                ...
                opts.cols.unshift({title:chkHtml,width: 20, align: 'center' ,lockWidth: true, checkCol: true, renderer:'checkCol_fix'});
            }

            if(opts.indexCol){
                opts.cols.unshift({title:'#',width: opts.indexColWidth, align: 'center' ,lockWidth: true, indexCol:true, renderer:'indexCol_fix'});
            }
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('">');
                    if(col.renderer){
                        var tmpR = renderDefault[col.renderer](item[col.name],item,rowIndex);
                        trHtml.push(tmpR);
                    }else{
