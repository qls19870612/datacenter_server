    ...
    MMGrid.prototype = {
        _initLayout: function($el){
            ...
            var mmGrid = [
                '<div class="mmGrid">',
                    ...
                    '<div class="mmg-message">'+ this.opts.noDataText +'</div>',
                    '<div class="mmg-mask mmg-transparent"></div>',
                    '<div class="mmg-page-controler"></div>',
                '</div>'
            ];
            ...
            this.$pageControler = $mmGrid.find('.mmg-page-controler');
        ...
        , _titleHtml: function(col, rowspan){
            ...
            if(!col.cols){
                ...
                if(!isNaN(Number(col.sortable))) col.sortable = Number(col.sortable);
        ...
        , _initHead: function(){
            ...
            var pgc_height = opts.pgInv > 0 ? 48 : 0;
            if(opts.height !== 'auto'){
                $bodyWrapper.height($mmGrid.height() - $headWrapper.outerHeight(true) - pgc_height);
            }
        ...
        , _initEvents: function(){
            ...
            var $body = this.$body;
            var $backboard = this.$backboard;
            var $pageControler = this.$pageControler;
            var $ths = this._expandThs();
            var expandCols = this._expandCols(opts.cols);
            ...
            //分页事件
            $pageControler.on('click', 'li', function(){
                var $this = $(this);
                var curPageNum = Number($(this).parent('ul').find('li.active:eq(0)').data('pagenum'));
                var totalPage = Number($(this).parent('ul').data('total'));
                if(!$this.hasClass('active')){
                    if($this.hasClass('pagenum')){
                        var selectPage = Number($this.data('pagenum'));
                        that._refreshPageControler(selectPage);
                        that._refreshPage(selectPage);

                    }else if($this.hasClass('pagectrl')){
                        var selectPage = 1;
                        if($this.data('pagectrl') == 'prev'){
                            selectPage = curPageNum-1 > 0 ? curPageNum-1 : 1;
                        }else if($this.data('pagectrl') == 'next'){
                            selectPage = curPageNum+1 < totalPage+1 ? curPageNum+1 : totalPage;
                        }else if($this.data('pagectrl') == 'jump'){
                            var inputPage = $this.prev('li.pageinput').find('input').val();
                            var inputPageNum = parseInt($.trim(inputPage));
                            if(inputPageNum < 0 || inputPageNum > totalPage || isNaN(inputPageNum)){
                                inputPageNum = totalPage;
                            }
                            selectPage = inputPageNum;
                        }
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            var expandCols = this._expandCols(opts.cols);
            var leafCols = this._leafCols();
            var pgInv = opts.pgInv || 0;


            if($.isPlainObject(item)){
                var trHtml = [];
                var rowTr = pgInv > 0 && rowIndex >= pgInv ? '<tr style="display:none">' : '<tr>';
                trHtml.push(rowTr);
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('">');
                    if(col.renderer){
                        trHtml.push(this._rendererDef(item[col.name],item,rowIndex,col.renderer));
                    }else{
        ...
        , load: function(args){
            try{
                ...
                if($.isArray(args)){
                    ...
                    this._pageControler(args.length);
                }else if(opts.url){
                    this._loadAjax(args);
                }else if(opts.items){
                    this._loadNative(opts.items);
                    this._pageControler(opts.items.length);
                }else{
                    this._loadNative([]);
                }
            }catch(e){
        ...
        ,_pageControler:function(total){
            var opts = this.opts;
            var pgInv = opts.pgInv;
            var totalPage =  Math.ceil(total/pgInv);
            var $pageControler = this.$pageControler;

            var pagectrler = ''
            , pagec_a = '<ul class="pagination" data-total="'+totalPage+'"><li class="pagectrl" data-pagectrl="prev"><a href="#">&laquo;</a></li>'
            , pagec_b = '<li class="pagectrl" data-pagectrl="next"><a href="#">&raquo;</a></li><li class="pageinput"><em style="padding:0"><input type="text" style="width: 40px; height: 34px; border: 1px solid #ddd; text-align: center;"></em></li><li class="pagectrl" data-pagectrl="jump"><a href="#">GO</a></li></ul>';
            ...
            if(totalPage < 8 && totalPage > 0){
                ...
                for(var i=0; i<totalPage; i++){
                    ...
                    tmp += '<li class="pagenum'+active+'" data-pagenum="'+pageNum+'"><a href="#">'+pageNum+'</a></li>';
                }
                pagectrler = pagec_a + tmp + pagec_b;
            }else if(totalPage >= 8){
                pagectrler = pagec_a + '<li class="pagenum active" data-pagenum="1"><a href="#">1</a></li><li class="pagenum" data-pagenum="2"><a href="#">2</a></li><li class="pagenum" data-pagenum="3"><a href="#">3</a></li><li><em>...</em></li><li class="pagenum" data-pagenum="'+totalPage+'"><a href="#">'+totalPage+'</a></li>' + pagec_b;
            }
            $pageControler.html(pagectrler);
        }

        ,_refreshPageControler:function(selectPage){
            var $pageControler = this.$pageControler;
            var $pagination = $pageControler.find('ul.pagination');
            var totalPage = Number($pagination.data('total')) || 0;
            var pageOption = ''
            , pageo_a = '<li class="pagectrl" data-pagectrl="prev"><a href="#">&laquo;</a></li>'
            , pageo_b = '<li class="pagectrl" data-pagectrl="next"><a href="#">&raquo;</a></li>'
            , pageo_c = '<li class="pageinput"><em style="padding:0"><input type="text" style="width: 40px; height: 34px; border: 1px solid #ddd; text-align: center;"></em></li><li class="pagectrl" data-pagectrl="jump"><a href="#">GO</a></li>';
            if(totalPage < 8 && totalPage > 0){
                var tmp = '';
                for(var i=0; i<totalPage; i++){
                    var pageNum = i+1;
                    var active = i == selectPage-1 ? ' active' : '';
                    tmp += '<li class="pagenum'+active+'" data-pagenum="'+pageNum+'"><a href="#">'+pageNum+'</a></li>';
                }
                pageOption = pageo_a + tmp + pageo_b + pageo_c;
            }else if(totalPage >= 8){
                var etc = '<li><em>...</em></li>';
                var tmp = '';
                var mkOption = function(idx, active){
                    var acti = active || 0;
                    var act = acti ? ' active' : '';
                    return '<li class="pagenum '+act+'" data-pagenum="'+idx+'"><a href="#">'+idx+'</a></li>';
                }
                switch(selectPage)
                {
                case 1:
                  tmp = mkOption(1, 1) + mkOption(2) + mkOption(3) + etc + mkOption(totalPage);
                  break;
                case 2:
                  tmp = mkOption(1) + mkOption(2, 1) + mkOption(3) + etc + mkOption(totalPage);
                  break;
                case totalPage-1:
                  tmp = mkOption(1) + etc + mkOption(totalPage-2) + mkOption(totalPage-1, 1) + mkOption(totalPage);
                  break;
                case totalPage:
                   tmp = mkOption(1) + etc + mkOption(totalPage-2) + mkOption(totalPage-1) + mkOption(totalPage, 1);
                  break;
                default:
                   var gap_a = '', gap_b = '';
                   if(selectPage-4 == 0){
                        gap_a = mkOption(selectPage-2);
                   }else if(selectPage-4 > 0 ){
                        gap_a = etc ;
                   }
                   if(selectPage+3 == totalPage){
                        gap_b = mkOption(selectPage+2);
                   }else if(selectPage+3 < totalPage){
                        gap_b = etc ;
                   }
                   var mid = mkOption(selectPage-1) + mkOption(selectPage, 1) + mkOption(selectPage+1);
                   tmp = mkOption(1) + gap_a + mid + gap_b + mkOption(totalPage);
                   break;
                }
                pageOption = pageo_a + tmp + pageo_b + pageo_c;
            }

            $pagination.html(pageOption);
        }

        , _refreshPage:function(selectPage){
            var opts = this.opts;
            var pginv = opts.pgInv;
            var $tbody = this.$bodyWrapper.find('tbody');
            $tbody.find('tr').hide();
            var startPage = (selectPage-1)*pginv;
            for(var i = startPage; i<startPage+pginv; i++){
        ...
        , _resizeHeight: function(){
            ...
            var $bodyWrapper = this.$bodyWrapper;
            var $body= this.$body;
            var pgc_height = opts.pgInv > 0 ? 45 : 0;
            if(opts.height === 'auto' && browser.isIE7){
                $bodyWrapper.height('auto');
                if($bodyWrapper.width() < $body.width()){
                    $bodyWrapper.height($bodyWrapper.height() + $bodyWrapper.height() - $bodyWrapper[0].clientHeight  + 1  - pgc_height);
                }
            }
        }
        ...
        , _rendererDef: function(val,item,index,red){
            var rtn = val;
            switch (red){
                case 'percentage':
                var tval = Number(val);
                if(!isNaN(tval)) rtn = '<b>'+tval.toFixed(2)+'%</b>';
                break;
                case 'fixed2':
                var tval = Number(val);
                if(!isNaN(tval)) rtn =  tval.toFixed(2);
                break;
            }
            return rtn;
        }
